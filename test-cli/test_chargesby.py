...
def test_chargesby_gen(capsys):
    ...
    output = f.read()
    process = Popen("python ../cli/src/se2160.py chargesby --op aodos --datefrom 20201001 --dateto 20201101", stdout=PIPE, stderr=PIPE)
...
def test_chargesby_f_operator(capsys):
    process = Popen("python ../cli/src/se2160.py chargesby --op kentri_odos --datefrom 20190101 --dateto 20190201", stdout=PIPE, stderr=PIPE)
    stdout, stderr = process.communicate()
...
def test_chargesby_f_date(capsys):
    process = Popen("python ../cli/src/se2160.py chargesby --op aodos --datefrom 20190101 --dateto 201902012", stdout=PIPE, stderr=PIPE)
    stdout, stderr = process.communicate()
    decoded_err = str(stderr.decode("utf-8"))[:-2]
    assert stdout == b''
