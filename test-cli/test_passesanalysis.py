...
def test_passesanalysis_f_sameop(capsys):
    process = Popen("python ../cli/se2160.py passesanalysis --op1 egnatia --op2 egnatia --datefrom 20201001 --dateto 20201101", stdout=PIPE, stderr=PIPE)
    stdout, stderr = process.communicate()
...
def test_passesanalysis_f_wrongop(capsys):
    process = Popen("python ../cli/se2160.py passesanalysis --op1 aodos --op2 egnat --datefrom 20201001 --dateto 20201101", stdout=PIPE, stderr=PIPE)
    stdout, stderr = process.communicate()
    decoded_err = str(stderr.decode("utf-8"))[:-2]
    assert stdout == b''
    assert decoded_err == 'egnat not a valid operator - check your input'
def test_passesanalysis_f_date(capsys):
    process = Popen("python ../cli/se2160.py passesanalysis --op1 aodos1 --op2 egnatia --datefrom 202010011 --dateto 2020110131", stdout=PIPE, stderr=PIPE)
    stdout, stderr = process.communicate()
    decoded_err = str(stderr.decode("utf-8"))[:-2]
    assert stdout == b''
